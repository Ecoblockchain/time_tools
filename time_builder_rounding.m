...
function time_series = time_builder_rounding(arg01, arg02, arg03, arg04, arg05, arg06, arg07, arg08, arg09, arg10, arg11, varargin)
...
if nargin >1 && nargin ~=4
    ...
    if nargin ~=6
        ...
        num_timesteps = round(day_range/serial_time_step);	% Round to nearest integer of time steps
        raw_timesteps = day_range/serial_time_step;	% Unrounded number of timesteps to check for evenly dividing time step into date range
		time_temp = zeros(num_timesteps+1,7);
		t_err = .001/(24.*60.*60); % Assume differences smaller than a millisecond are the result of rounding errors
		

        if num_timesteps <= raw_timesteps + t_err && num_timesteps >= raw_timesteps - t_err %then the number of timesteps in the range is exact (within rounding error)
